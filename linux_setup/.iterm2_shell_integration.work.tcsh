...
# Prevent the script from running twice.
  ...
  if ( $?loginsh) then
    ...
    if ( x"$ITERM_ENABLE_SHELL_INTEGRATION_WITH_TMUX""$TERM" != xscreen && x"$TERM" != xlinux && x"$TERM" != xdumb ) then
      ...
      alias _iterm2_print_shell_integration_version 'printf "1337;ShellIntegrationVersion=4;shell=tcsh"'
      ...
      #set seg_prompt = "`$CE_DIR/common/bin/make_prompt.py`"
      alias precmd 'set prompt="%{"`_iterm2_before_prompt`"%}`$CE_DIR/common/bin/make_prompt.py`%{"`_iterm2_after_prompt`"%} "; _iterm2_last_status; _iterm2_update_current_state '
